...
def get_zoom5_coords(W, H, w, h):
    ...
    return [
        ...
        (int(top_left_w),              int(H/2), int(w), int(h)),      # P4
        (int(top_left_w) + w,          int(H/2), int(w), int(h)),      # P5
        (int(top_left_w) + 2*w,        int(H/2), int(w), int(h)),      # P6
    ]

input_path = r"D:/2025신윤희영상정렬/24-2/F/F_W2_1234.mp4"
